...
# FIXME THIS SECTION IS BROKEN/DOESN'T MAKE SENSE FOR COASTWIDE AS IS - get seesaw
# Pick one species/survey/scenario
species <- "yelloweye-rockfish"
mpa_trend <- "mpa1.018"
ar1_scenario <- "fitted_AR1"
time_scenario <- "twenty-five_years"
plan <- "status-quo"  # or "mpas-at-5-year-intervals"

# Load conditioning model
fits <- load_cached_species(species, fit_dir = fit_dir)

# Load SAMPLED data (Stage 3) which implements the alternating survey schedule
sampled_files <- list.files(sample_dir, species,
  pattern = paste0("HBLL-.*_", mpa_trend, "_", ar1_scenario, "_", time_scenario, "_", plan, "\\.rds$"),
  full.names = TRUE
)

# Combine all three survey regions
sampled_dat <- purrr::map_dfr(sampled_files, readRDS)

# === Calculate historical index across all three survey regions ===
# For each survey, predict for years when that survey was actually conducted
hist_index <- purrr::map_dfr(fits, function(fit) {
  survey <- unique(fit$data$survey_abbrev)

  # Get prediction grid for this survey from sampled data
  # Use first year available for this survey
  pred_grid <- sampled_dat |>
    filter(survey_abbrev == survey, replicate == 1) |>
    filter(year == min(year)) |>
    distinct(survey_abbrev, X, Y, block_id, restricted)

  # Historical years for this survey (only years actually sampled)
  hist_years <- sort(unique(fit$data$year))

  # Create prediction data frame
  nd <- pred_grid |>
    expand_grid(year = hist_years) |>
    mutate(fyear = as.factor(year))

  # Get predictions (matrix: rows = observations, cols = simulations)
  set.seed(123)
  pred_matrix <- predict(fit, newdata = nd, nsim = 100, type = "response")

  # Take median across simulations for each grid cell × year
  nd$mu <- apply(pred_matrix, 1, median)

  # Return predictions with survey label and year
  nd |>
    select(year, restricted, mu, survey_abbrev)
}) |>
  # Sum across all grid cells within each survey × year × restricted
  # This respects that not all surveys run every year
  group_by(year, restricted) |>
  summarise(index = sum(mu), .groups = "drop") |>
  mutate(period = "historical")

# === Simulated indices (by restricted status) ===
# Use sampled data which implements the alternating schedule
sim_index <- sampled_dat |>
  group_by(year, restricted, replicate) |>
  summarise(index = sum(mu), .groups = "drop") |>
...
# === Scale each period separately ===
last_hist_year <- max(hist_index$year)
first_sim_year <- min(sim_index$year)

# Get baselines
hist_baseline <- hist_index |>
  filter(year == last_hist_year) |>
  select(restricted, hist_baseline = index)

sim_baseline <- sim_index |>
  filter(year == first_sim_year) |>
  select(restricted, sim_baseline = index)

# Scale each period by its own baseline
hist_scaled <- hist_index |>
  left_join(hist_baseline, by = "restricted") |>
  mutate(relative_index = index / hist_baseline) |>
  select(year, restricted, relative_index, period)

sim_scaled <- sim_index |>
  left_join(sim_baseline, by = "restricted") |>
  mutate(relative_index = index / sim_baseline) |>
  select(year, restricted, relative_index, period)

# Combine
all_indices <- bind_rows(hist_scaled, sim_scaled) |>
  mutate(area = ifelse(restricted == 1, "Inside MPA", "Outside MPA"))

# Plot
ggplot(all_indices, aes(x = year, y = relative_index, colour = area)) +
  geom_line(linewidth = 1) +
  geom_vline(xintercept = last_hist_year + 0.5, linetype = "dashed", alpha = 0.5) +
  geom_hline(yintercept = 1, linetype = "dotted", alpha = 0.5) +
